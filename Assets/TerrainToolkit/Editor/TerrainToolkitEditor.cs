...
public class TerrainToolkitEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        switch (terrain.toolModeInt)
        {
            ...
            // -------------------------------------------------------------------------------------------------------- TEXTURING TOOLS
            case 2:
                Terrain term = (Terrain)terrain.GetComponent(typeof(Terrain));
                if (term == null)
                {
                    return;
                }
                TerrainData termData = term.terrainData;
                terrain.splatPrototypes = termData.splatPrototypes;
                EditorGUILayout.Separator();
                float mouseX;
                EditorGUILayout.BeginHorizontal();
                GUI.skin = terrain.guiSkin;
                ...
                if (GUI.changed)
                {
                    termData.splatPrototypes = terrain.splatPrototypes;
                }
